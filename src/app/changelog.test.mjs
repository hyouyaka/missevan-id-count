...
test("changelog contains 1.7.3 revenue, rank, trend, and address updates", () => {
  const entry = CHANGELOG_ENTRIES.find((item) => item.version === "1.7.3");
...
test("changelog contains 1.7.1 anonymous feedback board update", () => {
  const entry = CHANGELOG_ENTRIES.find((item) => item.version === "1.7.1");

  assert.deepEqual(entry, {
    version: "1.7.1",
    changes: ["功能建议页面更新为匿名留言板模式方便交流"],
  });
});
...
test("changelog contains 1.6.1 search trend and mobile layout entry", () => {
  const entry = CHANGELOG_ENTRIES.find((item) => item.version === "1.6.1");
