...
Railway Columns 推荐添加 `category`、`event`、`platform`、`keywordText`、`keyResultText`、`taskType`、`source`。`keywordText` 汇总搜索词、趋势剧名、榜单/更新标题、从链接打开统计界面的剧名、CV 名称或统计任务剧名；`keyResultText` 仅记录成功统计任务的最终 ID 数、总播放量或收益数字，完整任务详情仍保留在 `result`。
