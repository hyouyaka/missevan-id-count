...
function createEpisodeDetail({
  ...
  const normalizedUniqueUsers = Number(uniqueUsers);
  const normalizedStatus = status === "failed" ? "failed" : "success";
  return {
    key: `${normalizedPlatform}:${normalizedDramaId}:${normalizedEpisodeId}`,
    ...
    totalDanmaku:
      totalDanmaku != null && Number.isFinite(normalizedTotal) && normalizedTotal >= 0
        ...
        : null,
    fetchedDanmaku:
      normalizedStatus === "success" && Number.isFinite(normalizedFetched) && normalizedFetched >= 0
        ? normalizedFetched
        : null,
    uniqueUsers:
      normalizedStatus === "success" && Number.isFinite(normalizedUniqueUsers) && normalizedUniqueUsers >= 0
        ? normalizedUniqueUsers
        : null,
  };
}

function getOrderedEpisodeDetails(detailsByKey, orderedKeys) {
  const details = detailsByKey instanceof Map ? detailsByKey : new Map();
  const detectedKeys = Array.from(details.keys());
  const detectedSet = new Set(detectedKeys);
  const emitted = new Set();
  const sortedKeys = [];
  (Array.isArray(orderedKeys) ? orderedKeys : []).forEach((key) => {
    if (!detectedSet.has(key) || emitted.has(key)) {
      return;
    }
    emitted.add(key);
    sortedKeys.push(key);
...
function buildEpisodeDetailKey(platform, dramaId, episodeId) {
  return `${String(platform ?? "").trim()}:${String(dramaId ?? "").trim()}:${String(episodeId ?? "").trim()}`;
}
...
export function createStatsTaskExecutor(dependencies = {}) {
  ...
  async function executeMissevanIdTask(task) {
    ...
    await runWithConcurrency(episodes, 1, async (episode) => {
      ...
      const detailKey = buildEpisodeDetailKey("missevan", dramaId, soundId);
      try {
    ...
    if (task.cancelled) {
      return finalizeCancelledTask(task, {
        ...
        result: {
          ...
          episodeDetails: Array.from(episodeDetails.values()),
  ...
  async function executeManboIdTask(task) {
    ...
    const allUsers = new Set();
    const episodeDetails = new Map();
    const episodeDetailOrder = episodes.map((episode) => buildEpisodeDetailKey(
      "manbo",
      episode?.drama_id,
      episode?.sound_id
    ));
    ...
    await runWithConcurrency(
      ...
      async (episode) => {
        ...
        const dramaTitle = String(episode?.drama_title ?? "").trim() || "Unknown";
        const episodeTitle = String(episode?.episode_title ?? episode?.name ?? "").trim();
        const detailKey = buildEpisodeDetailKey("manbo", dramaId, setId);
        try {
    ...
    if (task.cancelled) {
      return finalizeCancelledTask(task, {
        ...
        result: {
          ...
          episodeDetails: getOrderedEpisodeDetails(episodeDetails, episodeDetailOrder),
          totalDanmaku: task.totalDanmaku,
          totalUsers: allUsers.size,
          idSelectedEpisodeCount: task.totalCount,
        },
    ...
    for (const dramaIdValue of dramaIds) {
      ...
      try {
        ...
        for (let episodeIndex = 0; episodeIndex < paidEpisodes.length; episodeIndex += 1) {
          ...
          const detailKey = buildEpisodeDetailKey("missevan", dramaId, episode.sound_id);
          let danmakuResult;
          try {
            danmakuResult = await missevanClient.getDanmakuSummary(
              episode.sound_id,
              title,
              episodeTitle,
              task.source,
          ...
          if (danmakuResult.cancelled || task.cancelled || task.abortSignal?.aborted) {
            break;
          }
          ...
          (Array.isArray(danmakuResult.users) ? danmakuResult.users : []).forEach((uid) => {
            if (uid != null && uid !== "") {
              ...
              userSet.add(normalizedUid);
              episodeUserSet.add(normalizedUid);
            }
          });
    ...
    for (const dramaIdValue of dramaIds) {
      ...
      try {
        ...
        revenueEpisodes.forEach((episode) => {
          episodeDetailOrder.push(
            buildEpisodeDetailKey("manbo", dramaId, episode?.sound_id)
          );
        });
        const titlePricing = ["season", "episode"].includes(revenueType)
          ? resolveManboSeasonPricing(dramaInfo)
          : null;
        const seasonPricing = revenueType === "season" ? titlePricing : null;
        ...
        if (revenueType === "unknown") {
          ...
          const knownEpisodePrices = revenueEpisodes
            .map((episode) => Math.max(0, Number(episode?.price ?? 0) || 0))
          ...
          for (let episodeIndex = 0; episodeIndex < revenueEpisodes.length; episodeIndex += 1) {
            ...
            const episodeTitle = String(episode?.name ?? "").trim();
            const detailKey = buildEpisodeDetailKey("manbo", dramaId, episode.sound_id);
            let danmakuResult;
            try {
              danmakuResult = await manboClient.getDanmakuSummary(
                episode.sound_id,
                title,
                episodeTitle,
                task.source,
          ...
          if (revenueType === "member") {
            ...
            const normalizedPayCount = Math.max(0, Number(payCount ?? 0) || 0);
            const explicitTitlePrice = Math.max(
              0,
              Number(titlePricing?.titlePrice ?? 0) || 0
            );
            const episodePriceTotal = knownEpisodePrices.reduce(
              (sum, price) => sum + price,
              0
            );
            const titlePrice = explicitTitlePrice > 0
              ? explicitTitlePrice
              : episodePriceTotal;
            const memberPriceCandidate = Math.max(
              0,
              Number(dramaInfo?.drama?.member_price ?? 0) || 0
            );
            const normalizedDiamondValue = Math.max(0, Number(diamondValue) || 0);
            const minimumEpisodePrice = knownEpisodePrices.length > 0
              ? Math.min(...knownEpisodePrices)
              : 0;
            const hasPaidEpisodes = paidEpisodeCount > 0;
            const hasMemberDiscount =
              memberPriceCandidate > 0
              && titlePrice > 0
              && memberPriceCandidate < titlePrice;
            const minRevenueYuan = hasPaidEpisodes
              ? (
                  Math.max(
                    normalizedPayCount * minimumEpisodePrice,
                    episodePaidUserCountTotal * minimumEpisodePrice
                  ) + normalizedDiamondValue
                ) / 100
              : (
                  normalizedPayCount * (hasMemberDiscount ? memberPriceCandidate : titlePrice)
                  + normalizedDiamondValue
                ) / 100;
            const maxRevenueYuan = hasPaidEpisodes
              ? (
                  Math.max(
                    normalizedPayCount * titlePrice,
                    paidUserCount * titlePrice
                  ) + normalizedDiamondValue
                ) / 100
              : hasMemberDiscount
                ? (normalizedPayCount * titlePrice + normalizedDiamondValue) / 100
                : null;
            const hasRevenueRange = hasPaidEpisodes || hasMemberDiscount;
            results.push({
              dramaId,
              platform: "manbo",
              revenueType,
              ...
              viewCount,
              diamondValue,
              titlePrice: titlePrice > 0 ? titlePrice : null,
              titleMemberPrice: hasMemberDiscount ? memberPriceCandidate : null,
              includeInSummaryPrice: titlePrice > 0,
              currencyUnit: "红豆",
              summaryRevenueMode: hasRevenueRange ? "range" : "single",
              payCount: normalizedPayCount,
              paidCountSource: "pay_count_and_danmaku_ids",
              paidUserIds,
              paidUserCount,
              episodePaidUserCountTotal,
              seasonPaidUserCount: paidUserCount,
              estimatedRevenueYuan: minRevenueYuan,
              minRevenueYuan: hasRevenueRange ? minRevenueYuan : null,
              maxRevenueYuan,
              failed,
              accessDenied,
            });
